...
type AppGoogleBoundary = ReturnType<typeof JSON.parse>
type AppPropsBoundary = ReturnType<typeof JSON.parse>

declare global {
  interface Window {
    __tripCommandCenterMapsConfigured?: boolean
    google?: AppGoogleBoundary
  }
}
...
const PAGE_ICONS: Record<string, AppGoogleBoundary> = {
...
const WEATHER_ICONS: Record<string, AppGoogleBoundary> = {
  sun: Sun,
  partly: Cloud,
  cloud: Cloud,
  rain: CloudRain,
...
const DAY_BRIEFING_COPY: Record<string, AppGoogleBoundary> = {
  thu: {
...
const MISSION_LAUNCH_THEME: Record<string, AppGoogleBoundary> = {
  thu: {
    accent: '#F2CC60',
    accentStrong: '#FFD76B',
...
function getFamilyLabel(families: FamilyEntity[], familyId: string) {
  return families.find((family: AppGoogleBoundary) => family.id === familyId)?.title || 'Unknown family'
}
...
function buildEqualExpenseAllocations(amount: number, families: FamilyEntity[]): ExpenseAllocation[] {
  ...
  return families.map((family: AppGoogleBoundary,  index: AppGoogleBoundary) => ({
    familyId: family.id,
    title: family.title,
...
function getExpenseAllocations(expense: ExpenseEntity | null | undefined, families: FamilyEntity[]): ExpenseAllocation[] {
  ...
  if (expense.allocationMode === 'individual') {
    return families.map((family: AppGoogleBoundary) => ({
      familyId: family.id,
      title: family.title,
      amount: 0,
    }))
...
function buildManualAllocationSeed(amount: number, families: FamilyEntity[]): Record<string, number> {
  return Object.fromEntries(
    buildEqualExpenseAllocations(amount, families).map((item: AppGoogleBoundary) => [item.familyId, item.amount]),
  )
}

function getFamilyExpenseBurden(expenses: ExpenseEntity[], families: FamilyEntity[]): ExpenseAllocation[] {
  const totals = Object.fromEntries(families.map((family: AppGoogleBoundary) => [family.id, 0]))

  expenses.forEach((expense: AppGoogleBoundary) => {
    if (expense.allocationMode === 'individual') return
    getExpenseAllocations(expense, families).forEach((allocation: AppGoogleBoundary) => {
...
function getSuggestedPlaybackStartCursor(doc: TripDocument, cursorSlot: number, operationCheckpoints: TimelineGate[] = []) {
  const windows = (doc.routes || [])
    .map((route: AppGoogleBoundary) => getRouteSimulationWindow(doc, route))
    .filter((window: AppGoogleBoundary) => Number.isFinite(window.start) && Number.isFinite(window.end))
    .sort((left: AppGoogleBoundary,  right: AppGoogleBoundary) => left.start - right.start)
  const checkpoints = (operationCheckpoints || [])
    .filter((checkpoint: AppGoogleBoundary) => Number.isFinite(checkpoint?.startSlot))
    .sort((left: AppGoogleBoundary,  right: AppGoogleBoundary) => left.startSlot - right.startSlot)
  ...
  const activeWindow = windows.find((window: AppGoogleBoundary) => normalizedCursor >= window.start && normalizedCursor <= window.end)
  if (activeWindow) return normalizedCursor

  const nextWindow = windows.find((window: AppGoogleBoundary) => window.start > normalizedCursor)
  const nextCheckpoint = checkpoints.find((checkpoint: AppGoogleBoundary) => checkpoint.startSlot > normalizedCursor)
...
function dedupeById<T extends { id: string }>(items: (T | null | undefined)[]): T[] {
  const seen = new Set<AppGoogleBoundary>()
...
function pickMostFrequentEntity<T extends { id: string }>(items: T[]): T | null {
  ...
  items.forEach((item: AppGoogleBoundary) => {
    if (!item?.id) return
...
function getRelatedTravelItemsForGate(doc: TripDocument, gate: TimelineGate): ItineraryItemEntity[] {
  ...
  const gateEntityKeys = new Set(
    gateItems.flatMap((item: AppGoogleBoundary) => [makeEntityKey('itineraryItem', item.id), ...(item.linkedEntityKeys || [])]),
  )
  const sameDayTravelItems = doc.itineraryItems.filter((item: AppGoogleBoundary) => item.rowId === 'travel' && item.dayId === primaryItem.dayId)
  const directlyLinkedTravelItems = sameDayTravelItems.filter((item: AppGoogleBoundary) =>
    (item.linkedEntityKeys || []).some((key: AppGoogleBoundary) => gateEntityKeys.has(key)),
  )
  if (directlyLinkedTravelItems.length) return directlyLinkedTravelItems

  const launchWaveEnd = gate.startSlot + 1.2
  const sameWaveTravelItems = sameDayTravelItems.filter((item: AppGoogleBoundary) => {
  ...
  return sameDayTravelItems.filter((item: AppGoogleBoundary) => item.startSlot >= gate.startSlot - 0.25 && item.startSlot <= gate.startSlot + 0.55)
}
...
function buildOperationGateContext(doc: TripDocument, gate: TimelineGate | null) {
  ...
  const gateItemsWithType = gate.items.map((item: AppGoogleBoundary) => ({ ...item, type: 'itineraryItem' }))
  const linkedEntities = dedupeById(
    gateItemsWithType.flatMap((item: AppGoogleBoundary) => getLinkedEntities(doc, item)),
  )
  const relatedTravelItems = getRelatedTravelItemsForGate(doc, gate)
  const relatedRoutes = dedupeById(
    relatedTravelItems
      .map((item: AppGoogleBoundary) => getRouteForEntity(doc, { ...item, type: 'itineraryItem' }))
      .filter(Boolean),
  )
  const gateLocations = dedupeById(
    [
      getLocationForEntity(doc, { ...primaryItem, type: 'itineraryItem' }),
      ...linkedEntities.filter((entity: AppGoogleBoundary) => entity.type === 'location'),
      ...relatedRoutes
        .map((route: AppGoogleBoundary) => getTypedEntityById(doc, 'location', route.destinationLocationId))
        .filter(Boolean),
    ].filter(Boolean),
  )
  const targetLocation = pickMostFrequentEntity(gateLocations)
  const familyIds = [
    ...gate.items.flatMap((item: AppGoogleBoundary) => item.familyIds || []),
    ...relatedTravelItems.flatMap((item: AppGoogleBoundary) => item.familyIds || []),
    ...relatedRoutes.map((route: AppGoogleBoundary) => route.familyId).filter((familyId): familyId is string => Boolean(familyId && familyId !== 'all')),
  ]
  const families = dedupeById(
    familyIds
      .map((familyId: AppGoogleBoundary) => getTypedEntityById(doc, 'family', familyId))
      .filter(Boolean),
  )
  ...
  const etaSlot = relatedTravelItems.length
    ? Math.max(...relatedTravelItems.map((item: AppGoogleBoundary) => item.startSlot + getItineraryItemEffectiveSpan(doc, item)))
  ...
  const participantLabel =
    !families.length
      ...
      : families.length === doc.families.length
        ? 'All families'
        : formatNameList(families.map((family: AppGoogleBoundary) => family.title))
...
function buildOperationCheckpoints(doc: TripDocument): TimelineGate[] {
  return DAYS.map((day, dayIndex): TimelineGate | null => {
    const mainOp = doc.itineraryItems
      .filter((item: AppGoogleBoundary) => item.rowId === 'activities' && item.dayId === day.id)
      .sort((left: AppGoogleBoundary,  right: AppGoogleBoundary) => left.startSlot - right.startSlot)[0]
...
function findUpcomingOperationCheckpoint(checkpoints: TimelineGate[], cursorSlot: number, threshold = 0.14): TimelineGate | null {
  return checkpoints.find((item: AppGoogleBoundary) => item.startSlot >= cursorSlot && item.startSlot - cursorSlot <= threshold) || null
}
...
function findCrossedOperationCheckpoint(checkpoints: TimelineGate[], previousCursor: number, nextCursor: number, triggeredIds: Set<string>): TimelineGate | null {
  return checkpoints.find((item: AppGoogleBoundary) =>
...
function buildDailyBriefing(doc: TripDocument, context: TimelineContext): DailyBriefing {
  ...
  const meals = doc.meals.filter((meal: AppGoogleBoundary) => meal.dayId === day.id).slice(0, 3)
  const activities = doc.activities.filter((activity: AppGoogleBoundary) => activity.dayId === day.id).slice(0, 3)
  const tasks = getTasksForDay(doc, day.id).filter((task: AppGoogleBoundary) => task.status !== 'done').slice(0, 4)
  const liveItems = context.liveEntities.filter((item: AppGoogleBoundary) => item.dayId === day.id)
  const soonItems = [...context.nextEntities, ...context.prepSoon]
    .filter((item: AppGoogleBoundary) => item.dayId === day.id)
    .slice(0, 4)
...
function NotesBox({ value, onChange, placeholder }: { value: string; onChange: (value: string) => void; placeholder?: string }) {
  return (
    <textarea
      value={value}
      onChange={(event: AppGoogleBoundary) => onChange(event.target.value)}
...
function AppShell({
  ...
  searchResults: AppGoogleBoundary[]
  ...
  return (
    <div className="relative flex h-screen w-screen overflow-hidden bg-[#0d1117] font-sans text-[#C9D1D9] antialiased">
      <div className="flex w-16 flex-col border-r border-[#30363D] bg-[#0d1117]">
        ...
        {NAV_ITEMS.map((item: AppGoogleBoundary) => {
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1.5">
                {families.map((family: AppGoogleBoundary) => (
                  <button
                    key={family.id}
                    type="button"
            ...
            <div className="relative">
              ...
              <input
                ...
                onChange={(event: AppGoogleBoundary) => onSearchChange(event.target.value)}
                placeholder="Search..."
              ...
              {doc.ui.searchQuery && searchResults.length ? (
                <div className="absolute right-0 top-10 z-40 w-80 border border-[#30363D] bg-[#161b22] shadow-xl">
                  {searchResults.map((item: AppGoogleBoundary) => (
                    <button
...
function FamilyList({ doc, selection, onSelectEntity }: { doc: TripDocument; selection: EntitySelection; onSelectEntity: OpenEntityHandler }) {
  return (
    <div className="overflow-hidden border border-[#30363D] bg-[#0d1117]">
      {doc.families.map((family: AppGoogleBoundary) => {
...
function ScenarioControls({ doc, cursorSlot = doc.ui.timeline.cursorSlot, onSetCursor }: { doc: TripDocument; cursorSlot?: number; onSetCursor: (cursorSlot: number) => void }) {
  ...
  const selectedHour = MISSION_TIME_PRESETS.reduce((bestHour: AppGoogleBoundary,  hour: AppGoogleBoundary) => (
  ...
  return (
    <div className="border border-[#30363D] bg-[#161b22] p-4">
      ...
      <div className="mb-3 flex flex-wrap gap-2">
        {DAYS.map((day: AppGoogleBoundary,  dayIndex: AppGoogleBoundary) => (
          <button
            key={day.id}
            type="button"
      ...
      <div className="flex gap-2">
        {MISSION_TIME_PRESETS.map((hour: AppGoogleBoundary) => {
      ...
      <div className="p-4">
        {items.length ? (
          <div className="space-y-2">
            {items.map((item: AppGoogleBoundary) => (
              <button
                key={`${item.type}:${item.id}`}
                type="button"
                onClick={() => onOpenEntity(item.type, item.id)}
  ...
  return (
    ...
    >
      ...
      <div
        ...
        onClick={(event: AppGoogleBoundary) => event.stopPropagation()}
      >
        ...
        <div className="grid max-h-[calc(100vh-220px)] gap-5 overflow-y-auto p-6 xl:grid-cols-[1.05fr_0.95fr]">
          <div className="space-y-5">
            <div className="border border-[#30363D] bg-[#161b22] p-4">
              ...
              <div className="space-y-3">
                {briefing.lookouts.map((item: AppGoogleBoundary) => (
          ...
          <div className="space-y-5">
            <div className="border border-[#30363D] bg-[#161b22] p-4">
              ...
              <div className="space-y-3">
                {briefing.activities.map((activity: AppGoogleBoundary) => (
                  <button
                    key={activity.id}
                    type="button"
                ...
                {briefing.meals.map((meal: AppGoogleBoundary) => (
                  <button
                    key={meal.id}
                    type="button"
            ...
            <div className="border border-[#30363D] bg-[#161b22] p-4">
              ...
              <div className="space-y-2">
                {briefing.tasks.length ? briefing.tasks.map((task: AppGoogleBoundary) => (
                  <button
                    key={task.id}
                    type="button"
        ...
        <div className="relative border-b border-white/8 px-7 py-6">
          ...
          <div className="mt-5">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {statusCards.map((card: AppGoogleBoundary) => {
  ...
  return (
    <div className="border border-[#30363D] bg-[#161b22]">
      ...
      <div className="space-y-4 p-4">
        {sections.map((section: AppGoogleBoundary) => (
          <div key={section.title}>
            <div className="mb-2 text-[9px] font-black uppercase tracking-[0.18em] text-[#8B949E]">
              {section.title}
            </div>
            <div className="overflow-hidden border border-[#30363D] bg-[#0d1117]">
              {section.items.length ? section.items.map((item: AppGoogleBoundary) => (
                <button
                  key={`${item.type}:${item.id}`}
                  type="button"
                  onClick={() => onOpenEntity(item.type, item.id)}
...
function TimelineBoard({
  ...
  const days: TripDayWeather[] = weatherDays?.length
    ? weatherDays
    : DAYS.map((day: AppGoogleBoundary) => ({
      ...day,
  ...
  const rowLayouts = rows.map((row: AppGoogleBoundary,  index: AppGoogleBoundary) => ({
    ...row,
    height: rowHeights[row.id] || 40,
    top: rows.slice(0, index).reduce((sum: AppGoogleBoundary,  item: AppGoogleBoundary) => sum + (rowHeights[item.id] || 40), 0),
  }))
  const timelineHeight = rowLayouts.reduce((sum: AppGoogleBoundary,  row: AppGoogleBoundary) => sum + row.height, 0)
  const familyLaneMap = new Map(doc.families.map((family: AppGoogleBoundary,  index: AppGoogleBoundary) => [family.id, index]))
    ...
    <div className="shrink-0 border-t border-[#30363D] bg-[#161b22] shadow-[0_-8px_24px_rgba(0,0,0,0.35)]">
      ...
      <div className="flex" style={{ height: `${timelineHeight}px` }}>
        <div className="flex w-28 flex-col border-r border-[#30363D] bg-[#0d1117]/50">
          {rowLayouts.map((row: AppGoogleBoundary) => (
            <div
              key={row.id}
        ...
        <div
          ...
          onMouseDown={(event: AppGoogleBoundary) => {
          ...
          onMouseMove={(event: AppGoogleBoundary) => {
            const nextCursorSlot = scrubToClientX(event.clientX)
          ...
          onClick={(event: AppGoogleBoundary) => {
            const nextCursorSlot = scrubToClientX(event.clientX)
            if (nextCursorSlot != null) {
              onSetCursor?.(nextCursorSlot)
            }
        ...
        >
          <div className="absolute inset-0">
            {Array.from({ length: days.length * visibleHoursPerDay + 1 }).map((_: AppGoogleBoundary,  index: AppGoogleBoundary) => {
          ...
          <div className="absolute inset-0">
            {rowLayouts.map((row: AppGoogleBoundary) => {
              const rowItems = doc.itineraryItems.filter((item: AppGoogleBoundary) => item.rowId === row.id)
              ...
              return (
                ...
                >
                  {row.id === 'travel'
                    ? doc.families.slice(1).map((_: AppGoogleBoundary,  index: AppGoogleBoundary) => (
                        <div
                  ...
                  {rowItems.map((item: AppGoogleBoundary) => {
          ...
          {days.map((day: AppGoogleBoundary,  dayIndex: AppGoogleBoundary) => (
            <div key={day.id} className="relative flex flex-1 flex-col">
              ...
              <div className="flex h-full">
                {Array.from({ length: visibleHoursPerDay }).map((_: AppGoogleBoundary,  hourOffset: AppGoogleBoundary) => {
...
function IntelAction({ icon: Icon, label, onClick, tone = 'default' }: AppPropsBoundary) {
...
function InfoRow({ icon: Icon, label, value, muted = false }: AppPropsBoundary) {
...
function formatMealTravelSignal(meal: AppGoogleBoundary,  location: AppGoogleBoundary) {
...
function getMealContextNarrative(meal: AppGoogleBoundary,  location: AppGoogleBoundary,  linkedMission: AppGoogleBoundary) {
...
function getMealMedia(location: AppGoogleBoundary) {
  const seen = new Set<AppGoogleBoundary>()
  return [...(location?.livePhotos || []), ...(location?.photos || [])].filter((media: AppGoogleBoundary) => {
...
const ACTIVITY_RESEARCH: Record<string, AppGoogleBoundary> = {
  'thu-transit': {
...
const FAMILY_VEHICLE_DEFAULTS: Record<string, AppGoogleBoundary> = {
  'north-star': {
...
const ROUTE_SIM_DEFAULTS: Record<string, AppGoogleBoundary> = {
  'route-la-north-star': {
    originCoordinates: { lat: 34.1184, lng: -118.3004 },
...
function ActivityResearchCard({ eyebrow, title, bullets }: AppPropsBoundary) {
  return (
    <div className="border border-[#30363D] bg-[#0d1117] p-4">
      <SectionTitle eyebrow={eyebrow} title={title} />
      <div className="space-y-2">
        {bullets.map((bullet: AppGoogleBoundary) => (
...
function TransitStopCard({ stop, onSelectEntity }: AppPropsBoundary) {
  return (
    <button
      type="button"
...
function ItineraryPage({
  ...
  const [playbackCursorSlot, setPlaybackCursorSlot] = useState<AppGoogleBoundary>(null)
  const [isPlaybackPlaying, setIsPlaybackPlaying] = useState(false)
  const [playbackSpeed, setPlaybackSpeed] = useState(1)
  const [missionFeedItems, setMissionFeedItems] = useState<AppGoogleBoundary[]>([])
  const [missionFeedNow, setMissionFeedNow] = useState(() => Date.now())
  const [operationGate, setOperationGate] = useState<AppGoogleBoundary>(null)
  ...
  const operationGateRef = useRef<AppGoogleBoundary>(null)
  const triggeredOperationCheckpointIdsRef = useRef(new Set<AppGoogleBoundary>())
  ...
  const updateMissionFeedItems = useCallback((updater: AppGoogleBoundary) => {
    setMissionFeedItems((current: AppGoogleBoundary) => (typeof updater === 'function' ? updater(current) : updater))
  }, [])
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      updateMissionFeedItems((current: AppGoogleBoundary) => {
        const next = current.filter((item: AppGoogleBoundary) => now - (item.createdAt || 0) < expirationMs)
  ...
  const handlePlaybackFeedItems = useCallback((items: AppGoogleBoundary) => {
    ...
    updateMissionFeedItems((current: AppGoogleBoundary) => {
      const next = [...current]
      nextItems.forEach((item: AppGoogleBoundary) => {
        const nextItem = {
          ...item,
          createdAt,
        }
        const existingIndex = next.findIndex((existing: AppGoogleBoundary) => existing.key === item.key)
        if (existingIndex >= 0) {
  ...
  const handleMissionFeedActivate = useCallback((item: AppGoogleBoundary) => {
  ...
  const armOperationCheckpointsFromCursor = useCallback((cursorSlot: AppGoogleBoundary) => {
    const normalizedCursor = clampTimelineCursor(cursorSlot)
    triggeredOperationCheckpointIdsRef.current = new Set(
      operationCheckpoints
        .filter((checkpoint: AppGoogleBoundary) => checkpoint.startSlot <= normalizedCursor + 0.001)
        .map((checkpoint: AppGoogleBoundary) => checkpoint.id),
    )
  }, [operationCheckpoints])

  const triggerOperationGate = useCallback((checkpoint: AppGoogleBoundary) => {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: AppGoogleBoundary) => {
      if (event.key === 'Escape') {
  ...
  useEffect(() => {
    ...
    let frameId: AppGoogleBoundary = null
    ...
    const animate = (timestamp: AppGoogleBoundary) => {
    ...
    <>
          ...
          <div className="space-y-4 p-4">
            ...
            <div>
              <PageNotesCard
                ...
                onChange={(value: AppGoogleBoundary) => onUpdatePageNote('itinerary', value)}
      ...
      {briefingOpen ? (
        <DailyBriefingModal
          ...
          onOpenEntity={(type: AppGoogleBoundary,  id: AppGoogleBoundary) => {
...
function StayPage({ doc, selection, onSelectEntity, onUpdatePageNote, onConvertPageNote }: AppPropsBoundary) {
  ...
  return (
    <div className="grid min-h-0 flex-1 grid-cols-[minmax(380px,440px)_1fr] overflow-hidden">
      <div className="overflow-y-auto border-r border-[#30363D] bg-[#161b22] p-6">
        ...
        <div className="space-y-4">
          {doc.stayItems.map((item: AppGoogleBoundary) => (
            <SelectableCard
              key={item.id}
      ...
      <div className="overflow-y-auto bg-[#0d1117] p-6">
          ...
          <div className="border border-[#30363D] bg-[#161b22] p-4">
            <div className="mb-3 flex items-center justify-between gap-3">
              ...
              {showExternalListing ? (
                <button
                  type="button"
                  onClick={() => (window.open as AppGoogleBoundary)(airbnb.externalUrl, '_blank', 'noreferrer')}
            ...
            <div className="mb-3 grid gap-3 sm:grid-cols-2">
              {(airbnb.photos || []).slice(0, 2).map((media: AppGoogleBoundary) => (
                <a
                  key={media.id}
            ...
            <div className="space-y-2">
              {showManual ? (
                <button
                  type="button"
                  onClick={() => (window.open as AppGoogleBoundary)(airbnb.manualUrl, '_blank', 'noreferrer')}
        ...
        <div className="mb-6 grid gap-4 md:grid-cols-2">
          {doc.families.map((family: AppGoogleBoundary,  index: AppGoogleBoundary) => (
            <SelectableCard
              key={family.id}
        ...
        <PageNotesCard
          ...
          onChange={(value: AppGoogleBoundary) => onUpdatePageNote('stay', value)}
...
function MealsPage({ doc, selection, onSelectEntity, onToggleMealStatus, onUpdatePageNote, onConvertPageNote }: AppPropsBoundary) {
  ...
  const selectedTasks = getTasksForEntity(doc, selectedMeal).filter((task: AppGoogleBoundary) => task.status !== 'done').slice(0, 2)
  const linkedMission = getLinkedEntities(doc, selectedMeal).find(
    (entity: AppGoogleBoundary) => entity.type === 'activity' || entity.type === 'itineraryItem',
  )
  ...
  return (
    <div className="grid min-h-0 flex-1 grid-cols-[minmax(360px,0.78fr)_minmax(520px,1.22fr)] overflow-hidden">
      <div className="overflow-y-auto border-r border-[#30363D] bg-[#161b22] p-6">
        ...
        <div className="space-y-3">
          {doc.meals.map((meal: AppGoogleBoundary) => (
            <div
              key={meal.id}
              className={cn(
          ...
          <>
              ...
              <div className="mb-4 flex flex-wrap gap-2">
                {selectedLocation?.externalUrl ? (
                  <IntelAction
                    ...
                    onClick={() => (window.open as AppGoogleBoundary)(selectedLocation.externalUrl, '_blank', 'noreferrer')}
                  />
                ) : null}
                {selectedLocation?.websiteUrl ? (
                  <IntelAction
                    icon={Globe}
                    label="Venue website"
                    onClick={() => (window.open as AppGoogleBoundary)(selectedLocation.websiteUrl, '_blank', 'noreferrer')}
                  />
                ) : null}
                {linkedMission ? (
                  <IntelAction
                  ...
                  <div className="space-y-3">
                    ...
                    {selectedTasks.length ? (
                      <div className="rounded-[2px] border border-[#30363D] bg-[#161b22] px-3 py-3">
                        ...
                        <div className="space-y-2">
                          {selectedTasks.map((task: AppGoogleBoundary) => (
            ...
            {media.length ? (
              <div className="mt-5 border border-[#30363D] bg-[#161b22] p-5">
                ...
                <div className="grid gap-4 md:grid-cols-3">
                  {media.map((item: AppGoogleBoundary) => (
                    <a
                      key={item.id}
            ...
            <div className="mt-5">
              <PageNotesCard
                ...
                onChange={(value: AppGoogleBoundary) => onUpdatePageNote('meals', value)}
...
function ActivitiesPage({ doc, selection, onSelectEntity, onUpdatePageNote, onConvertPageNote, onAddActivity }: AppPropsBoundary) {
  const selectedActivity = useMemo(
    () => (selection.type === 'activity' ? doc.activities.find((activity: AppGoogleBoundary) => activity.id === selection.id) || doc.activities[0] : doc.activities[0]),
  ...
  const linkedTimelineItems = useMemo(
    () => linkedEntities.filter((entity: AppGoogleBoundary) => entity.type === 'itineraryItem'),
    [linkedEntities],
  )
  ...
  const transitFamilies = useMemo(() => {
    ...
    return linkedTimelineItems
      .filter((item: AppGoogleBoundary) => item.familyIds?.length === 1)
      .map((item: AppGoogleBoundary) => {
        ...
        const stops = (route?.stopLocationIds || [])
          .map((stopId: AppGoogleBoundary) => getTypedEntityById(doc, 'location', stopId))
          .filter(Boolean)
  ...
  const [selectedTransitFamilyId, setSelectedTransitFamilyId] = useState<AppGoogleBoundary>(null)
  useEffect(() => {
    ...
    if (!transitFamilies.some((entry: AppGoogleBoundary) => entry.family.id === selectedTransitFamilyId)) {
      setSelectedTransitFamilyId((transitFamilies[0] as AppGoogleBoundary).family.id)
    }
  }, [selectedTransitFamilyId, transitFamilies])
  const selectedTransitPlan = useMemo(
    () => transitFamilies.find((entry: AppGoogleBoundary) => entry.family.id === selectedTransitFamilyId) || transitFamilies[0] || null,
    [selectedTransitFamilyId, transitFamilies],
  )
  ...
  return (
    <div className="grid min-h-0 flex-1 grid-cols-[360px_minmax(560px,1fr)] overflow-hidden">
      <div className="overflow-y-auto border-r border-[#30363D] bg-[#161b22] p-6">
        ...
        <div className="space-y-4">
          {doc.activities.map((activity: AppGoogleBoundary) => (
            <SelectableCard
              key={activity.id}
        ...
        <div className="mt-5 border border-[#30363D] bg-[#0d1117] p-4">
          ...
          <div className="space-y-3">
            <input
              value={draftTitle}
              onChange={(event: AppGoogleBoundary) => setDraftTitle(event.target.value)}
            ...
            <div className="grid grid-cols-[110px_1fr] gap-2">
              <select
                value={draftDayId}
                onChange={(event: AppGoogleBoundary) => setDraftDayId(event.target.value)}
              ...
              <input
                value={draftWindow}
                onChange={(event: AppGoogleBoundary) => setDraftWindow(event.target.value)}
          ...
          <>
            ...
            {selectedActivity.id === 'thu-transit' && selectedTransitPlan ? (
              <div className="mt-5 border border-[#30363D] bg-[#161b22] p-5">
                ...
                <div className="mb-4 flex flex-wrap gap-2">
                  {transitFamilies.map((entry: AppGoogleBoundary) => (
                    <button
                      key={entry.family.id}
                      type="button"
                      onClick={() => {
                  ...
                  <div className="border border-[#30363D] bg-[#0d1117] p-4">
                    ...
                    {selectedTransitPlan.stops.length ? (
                      <div className="grid gap-3 md:grid-cols-2">
                        {selectedTransitPlan.stops.map((stop: AppGoogleBoundary) => (
              ...
              <div className="mt-5 grid gap-4 xl:grid-cols-2">
                {research.cards.map((card: AppGoogleBoundary) => (
            ...
            <div className="mt-5">
              <PageNotesCard
                ...
                onChange={(value: AppGoogleBoundary) => onUpdatePageNote('activities', value)}
...
function ExpensesPage({
  ...
  const activeExpenseId =
    selection.type === 'expense' && doc.expenses.some((expense: AppGoogleBoundary) => expense.id === selection.id)
      ? selection.id
      : doc.expenses[0]?.id
  const activeExpense = doc.expenses.find((expense: AppGoogleBoundary) => expense.id === activeExpenseId) || null
  const [amountDraft, setAmountDraft] = useState('')
  const [manualAllocationDrafts, setManualAllocationDrafts] = useState<AppGoogleBoundary>({})
  const [customPayerDraft, setCustomPayerDraft] = useState('')
  const total = useMemo(() => doc.expenses.reduce((sum: AppGoogleBoundary,  expense: AppGoogleBoundary) => sum + expense.amount, 0), [doc.expenses])
  const outstanding = useMemo(
    () => doc.expenses.filter((expense: AppGoogleBoundary) => !expense.settled).reduce((sum: AppGoogleBoundary,  expense: AppGoogleBoundary) => sum + expense.amount, 0),
    [doc.expenses],
  )
  ...
  const manualAllocatedTotal = useMemo(
    () => activeAllocations.reduce((sum: AppGoogleBoundary,  allocation: AppGoogleBoundary) => sum + allocation.amount, 0),
    [activeAllocations],
  )
  ...
  const payerOptions = useMemo(
    () => [
      ...doc.families.map((family: AppGoogleBoundary) => family.title),
      'Each family',
      'Unassigned',
    ],
    [doc.families],
  ...
  useEffect(() => {
    ...
    setManualAllocationDrafts(
      Object.fromEntries(
        getExpenseAllocations(activeExpense, doc.families).map((allocation: AppGoogleBoundary) => [
          allocation.familyId,
  ...
  const commitManualAllocationDraft = useCallback((familyId: AppGoogleBoundary) => {
    if (!activeExpense) return
    ...
    setManualAllocationDrafts((current: AppGoogleBoundary) => ({
      ...current,
  ...
  return (
    <div className="grid min-h-0 flex-1 grid-cols-[minmax(420px,0.95fr)_minmax(440px,1.05fr)] overflow-hidden">
      <div className="overflow-y-auto border-r border-[#30363D] bg-[#161b22] p-6">
        ...
        <div className="border border-[#30363D] bg-[#0d1117]">
          {doc.expenses.map((expense: AppGoogleBoundary) => (
            <div
              key={expense.id}
              role="button"
              tabIndex={0}
              onClick={() => onSelectEntity('expense', expense.id)}
              onKeyDown={(event: AppGoogleBoundary) => {
      ...
      <div className="overflow-y-auto bg-[#0d1117] p-6">
            ...
            <div className="mb-6 grid gap-4">
              <div className="grid gap-3 md:grid-cols-2">
                <label className="grid gap-1.5">
                  ...
                  <input
                    ...
                    onChange={(event: AppGoogleBoundary) => onUpdateExpenseFields(activeExpense.id, { title: event.target.value })}
                    className="border border-[#30363D] bg-[#161b22] px-3 py-2 text-[11px] text-[#C9D1D9] outline-none focus:border-[#58A6FF]"
                  />
                </label>
                <label className="grid gap-1.5">
                  ...
                  <div className="grid gap-2">
                    <select
                      value={payerMode}
                      onChange={(event: AppGoogleBoundary) => {
                      ...
                      className="border border-[#30363D] bg-[#161b22] px-3 py-2 text-[11px] text-[#C9D1D9] outline-none focus:border-[#58A6FF]"
                    >
                      {payerOptions.map((option: AppGoogleBoundary) => (
                    ...
                    {payerMode === '__custom__' ? (
                      <input
                        ...
                        onChange={(event: AppGoogleBoundary) => setCustomPayerDraft(event.target.value)}
                ...
                <label className="grid gap-1.5">
                  ...
                  <input
                    inputMode="decimal"
                    value={amountDraft}
                    onChange={(event: AppGoogleBoundary) => setAmountDraft(event.target.value)}
                    onBlur={commitAmountDraft}
                    onFocus={(event: AppGoogleBoundary) => event.target.select()}
                    placeholder="0"
                    className="border border-[#30363D] bg-[#161b22] px-3 py-2 text-[11px] text-[#C9D1D9] outline-none focus:border-[#58A6FF]"
                  />
                </label>
              ...
              <div className="border border-[#30363D] bg-[#161b22] p-4">
                ...
                {activeExpense.allocationMode === 'individual' ? (
                  ...
                  <div className="grid gap-2">
                    {activeAllocations.map((allocation: AppGoogleBoundary) => (
                      <div
              ...
              <label className="grid gap-1.5">
                ...
                <textarea
                  ...
                  onChange={(event: AppGoogleBoundary) => onUpdateExpenseFields(activeExpense.id, { note: event.target.value })}
                  rows={4}
        ...
        <div className="mb-6 border border-[#30363D] bg-[#161b22] p-4">
          ...
          <div className="grid gap-2">
            {familyBurden.map((entry: AppGoogleBoundary) => (
              <div
                key={entry.familyId}
        ...
        <PageNotesCard
          ...
          onChange={(value: AppGoogleBoundary) => onUpdatePageNote('expenses', value)}
...
function FamiliesPage({ doc, selection, onSelectEntity, onUpdatePageNote, onConvertPageNote }: AppPropsBoundary) {
  return (
      ...
      <div className="overflow-y-auto border-r border-[#30363D] bg-[#161b22] p-6">
        ...
        <div className="mt-5">
          <PageNotesCard
            ...
            onChange={(value: AppGoogleBoundary) => onUpdatePageNote('families', value)}
          ...
          {doc.families.map((family: AppGoogleBoundary) => {
            ...
            return (
              ...
              >
                ...
                <div className="space-y-2">
                  {tasks.map((task: AppGoogleBoundary) => (
...
function withRefreshedFamilies(nextDoc: AppGoogleBoundary) {
  return {
    ...nextDoc,
    families: nextDoc.families.map((family: AppGoogleBoundary) => ({
      ...family,
...
function App() {
  ...
  const locationIntelHydrationRef = useRef(new Set<AppGoogleBoundary>())
  const startupTimelineSyncRef = useRef(false)
  const seededPlanRefreshRef = useRef(false)
  const [weatherState, setWeatherState] = useState<AppGoogleBoundary>({
    status: 'loading',
    targets: {},
    updatedAt: null,
    error: null,
  ...
  const currentFamily = displayDoc.families.find((family: AppGoogleBoundary) => family.id === viewerProfile?.familyId) || null
  ...
  const setActiveFamilyProfile = useCallback((familyId: AppGoogleBoundary) => {
  ...
  useEffect(() => {
    ...
    setDoc((current: AppGoogleBoundary) => ({
      ...current,
      ui: {
        ...current.ui,
        timeline: {
  ...
  useEffect(() => {
    const jiangRoute = doc.routes.find((route: AppGoogleBoundary) => route.id === 'route-la-north-star')
    const jiangFamily = doc.families.find((family: AppGoogleBoundary) => family.id === 'north-star')
    const yosemiteLocation = doc.locations.find((location: AppGoogleBoundary) => location.id === 'yosemite')
    const missingStopLocations = JIANG_ROAD_TRIP_STOP_DEFAULTS.filter(
      (stop: AppGoogleBoundary) => !doc.locations.some((location: AppGoogleBoundary) => location.id === stop.id),
    )
    ...
    const needsVehicleFamilyBackfill = doc.families.some((family: AppGoogleBoundary) => {
    ...
    const needsVehicleRouteBackfill = doc.routes.some((route: AppGoogleBoundary) => {
    ...
    setDoc((current: AppGoogleBoundary) => {
      const nextLocations = [
        ...
        ...JIANG_ROAD_TRIP_STOP_DEFAULTS.filter(
          (stop: AppGoogleBoundary) => !current.locations.some((location: AppGoogleBoundary) => location.id === stop.id),
        ),
      ].map((location: AppGoogleBoundary) =>
      ...
      const nextFamilies = current.families.map((family: AppGoogleBoundary) => {
        const defaults = FAMILY_VEHICLE_DEFAULTS[family.id]
      ...
      const nextRoutes = synchronizeRoutePaths(
        current.routes.map((route: AppGoogleBoundary) => {
          const defaults = ROUTE_SIM_DEFAULTS[route.id]
  ...
  useEffect(() => {
    ...
    const currentById = (collection: AppGoogleBoundary) => new Map(collection.map((item: AppGoogleBoundary) => [item.id, item]))
    const collectionNeedsRefresh = (currentCollection: AppGoogleBoundary,  initialCollection: AppGoogleBoundary,  refreshIds: AppGoogleBoundary) => {
      const currentMap = currentById(currentCollection)
      const initialMap = currentById(initialCollection)
      return [...refreshIds].some((id: AppGoogleBoundary) => {
    ...
    const missingRoutes = initialDoc.routes.filter((route: AppGoogleBoundary) => !doc.routes.some((currentRoute: AppGoogleBoundary) => currentRoute.id === route.id))
    const missingItineraryItems = initialDoc.itineraryItems.filter(
      (item: AppGoogleBoundary) => !doc.itineraryItems.some((currentItem: AppGoogleBoundary) => currentItem.id === item.id),
    )
    const hasObsoleteRoutes = doc.routes.some((route: AppGoogleBoundary) => OBSOLETE_PLAN_ROUTE_IDS.has(route.id))
    const hasObsoleteItineraryItems = doc.itineraryItems.some((item: AppGoogleBoundary) => OBSOLETE_PLAN_ITINERARY_IDS.has(item.id))
    ...
    seededPlanRefreshRef.current = true
    setDoc((current: AppGoogleBoundary) => {
  const syncCollection = (currentCollection: AppGoogleBoundary[], initialCollection: AppGoogleBoundary[], refreshIds: Set<AppGoogleBoundary>, obsoleteIds = new Set<AppGoogleBoundary>()) => {
        const initialMap = new Map(initialCollection.map((item: AppGoogleBoundary) => [item.id, item]))
        const filtered = currentCollection.filter((item: AppGoogleBoundary) => !obsoleteIds.has(item.id))
        const existingIds = new Set(filtered.map((item: AppGoogleBoundary) => item.id))
        const replaced = filtered.map((item: AppGoogleBoundary) => (refreshIds.has(item.id) && initialMap.has(item.id) ? initialMap.get(item.id) : item))
        const additions = [...refreshIds]
          .filter((id: AppGoogleBoundary) => !existingIds.has(id) && initialMap.has(id))
          .map((id: AppGoogleBoundary) => initialMap.get(id))
      ...
      const nextRoutes = synchronizeRoutePaths(
        syncCollection(
          [
            ...current.routes,
            ...initialDoc.routes.filter((route: AppGoogleBoundary) => !current.routes.some((currentRoute: AppGoogleBoundary) => currentRoute.id === route.id)),
          ],
          initialDoc.routes,
      ...
      const nextItineraryItems = syncCollection(
        [
          ...
          ...initialDoc.itineraryItems.filter((item: AppGoogleBoundary) => !current.itineraryItems.some((currentItem: AppGoogleBoundary) => currentItem.id === item.id)),
        ],
  ...
  const timelineWeatherDays = useMemo(
    () => DAYS.map((day: AppGoogleBoundary) => ({ ...day, ...getTripDayWeather(weatherState.targets, day) })),
  ...
  const setSelectedPage = useCallback((pageId: AppGoogleBoundary) => {
    setDoc((current: AppGoogleBoundary) => ({
      ...current,
      selectedPage: pageId,
  ...
  const selectEntity = useCallback((type: AppGoogleBoundary,  id: AppGoogleBoundary) => {
    setDoc((current: AppGoogleBoundary) => {
  ...
  const openEntity = useCallback((type: AppGoogleBoundary,  id: AppGoogleBoundary) => {
    setDoc((current: AppGoogleBoundary) => ({
      ...current,
      selection: { type, id },
      ui: { ...current.ui, searchQuery: '' },
  ...
  const hydrateLocationDetails = useCallback((locationId: AppGoogleBoundary,  patch: AppGoogleBoundary) => {
    if (!locationId || !patch) return

    setDoc((current: AppGoogleBoundary) => {
      let changed = false
      const locations = current.locations.map((location: AppGoogleBoundary) => {
  ...
  const hydrateRouteDetails = useCallback((routeId: AppGoogleBoundary,  patch: AppGoogleBoundary) => {
    if (!routeId || !patch) return

    setDoc((current: AppGoogleBoundary) => {
      let changed = false
      const routes = current.routes.map((route: AppGoogleBoundary) => {
  ...
  const updateLocationFields = useCallback((locationId: AppGoogleBoundary,  patch: AppGoogleBoundary) => {
    setDoc((current: AppGoogleBoundary) => {
      const locations = current.locations.map((location: AppGoogleBoundary) =>
  ...
  useEffect(() => {
    ...
    const basecampLocation = doc.locations.find((location: AppGoogleBoundary) => location.id === 'pine-airbnb')
    const pendingPlaceLocations = doc.locations.filter((location: AppGoogleBoundary) => {
    ...
    async function hydrateMealIntel() {
      try {
        if (!window.__tripCommandCenterMapsConfigured) {
          (setOptions as AppGoogleBoundary)({
        ...
        const findPlaceMatch = (location: AppGoogleBoundary) =>
          new Promise<AppGoogleBoundary>((resolve: AppGoogleBoundary,  reject: AppGoogleBoundary) => {
            ...
            placesService.findPlaceFromQuery(
              ...
              (results: AppGoogleBoundary,  status: AppGoogleBoundary) => {
        ...
        const fetchPlaceDetails = (placeId: AppGoogleBoundary) =>
          new Promise<AppGoogleBoundary>((resolve: AppGoogleBoundary,  reject: AppGoogleBoundary) => {
            if (!placeId) {
              resolve(null)
              return
            ...
            placesService.getDetails(
              ...
              (result: AppGoogleBoundary,  status: AppGoogleBoundary) => {
        ...
        const fetchDriveProfile = (origin: AppGoogleBoundary,  destination: AppGoogleBoundary) =>
          new Promise<AppGoogleBoundary>((resolve: AppGoogleBoundary,  reject: AppGoogleBoundary) => {
        ...
        for (const location of pendingPlaceLocations) {
          ...
          try {
            ...
            const livePhotos = (placeDetails?.photos || []).slice(0, 3).map((photo: AppGoogleBoundary,  index: AppGoogleBoundary) => ({
  ...
  useEffect(() => {
    ...
    const basecamp = doc.locations.find((location: AppGoogleBoundary) => location.id === 'pine-airbnb')
    const yosemite = doc.locations.find((location: AppGoogleBoundary) => location.id === 'yosemite')
    ...
    const loadWeather = async () => {
      try {
        ...
        if (cancelled) return
        setWeatherState((current: AppGoogleBoundary) => ({
          ...current,
          status: 'error',
          error: (error as AppGoogleBoundary)?.message || 'Weather fetch failed',
        }))
      }
    }
  ...
  const updatePageNote = (pageId: AppGoogleBoundary,  value: AppGoogleBoundary) => {
    setDoc((current: AppGoogleBoundary) => ({
      ...current,
  ...
  const updateEntityNote = (type: AppGoogleBoundary,  id: AppGoogleBoundary,  value: AppGoogleBoundary) => {
    setDoc((current: AppGoogleBoundary) => {
      const collectionName = ({
        ...
        expense: 'expenses',
        task: 'tasks',
      } as AppGoogleBoundary)[type]
      if (!collectionName) return current
      return {
        ...current,
        [collectionName]: updateEntityInCollection(current[collectionName], id, (item: AppGoogleBoundary) => ({
  ...
  const toggleTask = (taskId: AppGoogleBoundary) => {
    setDoc((current: AppGoogleBoundary) => {
      const nextDoc = {
        ...current,
        tasks: current.tasks.map((task: AppGoogleBoundary) =>
          task.id === taskId
  ...
  const addTask = (entityType: AppGoogleBoundary,  entityId: AppGoogleBoundary,  title: AppGoogleBoundary) => {
    setDoc((current: AppGoogleBoundary) => {
      ...
      const collectionName = ({
        ...
        expense: 'expenses',
        task: 'tasks',
      } as AppGoogleBoundary)[entityType]
      ...
      const nextDoc = {
        ...
        [collectionName]:
          entityType === 'task'
            ...
            : updateEntityInCollection(current[collectionName], entityId, (item: AppGoogleBoundary) => ({
                ...item,
  ...
  const addActivity = ({ title, dayId, window, description }: AppPropsBoundary) => {
  ...
  const convertNoteToTask = (entityType: AppGoogleBoundary,  entityId: AppGoogleBoundary) => {
  ...
  const convertPageNoteToTask = (pageId: AppGoogleBoundary) => {
    ...
    if (!note.trim()) return
      ...
      const entityType = (pageToEntityType as AppGoogleBoundary)[pageId]
      const collectionName = (ENTITY_PAGE as AppGoogleBoundary)[entityType] ? ({
      activity: 'activities',
      stayItem: 'stayItems',
      meal: 'meals',
      expense: 'expenses',
      family: 'families',
      } as AppGoogleBoundary)[entityType] : null
      const target = collectionName ? (doc as AppGoogleBoundary)[collectionName]?.[0] : null
    if (!target) return
    addTask(target.type, target.id, note.trim().split('\n')[0].slice(0, 96))
  }

  const toggleMealStatus = (mealId: AppGoogleBoundary) => {
    setDoc((current: AppGoogleBoundary) => ({
      ...current,
      meals: current.meals.map((meal: AppGoogleBoundary) =>
        meal.id === mealId
  ...
  const toggleExpenseSettled = (expenseId: AppGoogleBoundary) => {
    setDoc((current: AppGoogleBoundary) => ({
      ...current,
      expenses: current.expenses.map((expense: AppGoogleBoundary) =>
  ...
  const updateExpenseFields = (expenseId: AppGoogleBoundary,  patch: AppGoogleBoundary) => {
    setDoc((current: AppGoogleBoundary) => ({
      ...current,
      expenses: current.expenses.map((expense: AppGoogleBoundary) => {
  ...
  const setExpenseAllocationMode = (expenseId: AppGoogleBoundary,  allocationMode: AppGoogleBoundary) => {
    setDoc((current: AppGoogleBoundary) => ({
      ...current,
      expenses: current.expenses.map((expense: AppGoogleBoundary) => {
        if (expense.id !== expenseId) return expense
  ...
  const updateExpenseAllocation = (expenseId: AppGoogleBoundary,  familyId: AppGoogleBoundary,  amount: AppGoogleBoundary) => {
    setDoc((current: AppGoogleBoundary) => ({
      ...current,
      expenses: current.expenses.map((expense: AppGoogleBoundary) =>
        expense.id === expenseId
  ...
  const resetExpenseAllocationsToEqual = (expenseId: AppGoogleBoundary) => {
    setDoc((current: AppGoogleBoundary) => ({
      ...current,
      expenses: current.expenses.map((expense: AppGoogleBoundary) =>
        expense.id === expenseId
  ...
  const updateMapUi = (patch: AppGoogleBoundary) => {
    setDoc((current: AppGoogleBoundary) => ({
      ...current,
      ui: {
        ...current.ui,
  ...
  const setTimelineCursor = useCallback((cursorSlot: AppGoogleBoundary) => {
    setDoc((current: AppGoogleBoundary) => ({
      ...current,
      ui: {
        ...current.ui,
  ...
  const updateSearchQuery = (searchQuery: AppGoogleBoundary) => {
    setDoc((current: AppGoogleBoundary) => ({
      ...current,
      ui: { ...current.ui, searchQuery },
    }))
